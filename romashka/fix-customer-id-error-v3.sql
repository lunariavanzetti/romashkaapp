-- Fix for customer_id column error - Version 3
...
-- Verify the fix worked (fixed ambiguous column reference)
SELECT 
  'customer_profiles' as table_check,
  CASE WHEN ist.table_name IS NOT NULL THEN 'EXISTS' ELSE 'MISSING' END as status
FROM information_schema.tables ist
WHERE ist.table_schema = 'public' AND ist.table_name = 'customer_profiles'
UNION ALL
SELECT 
  'customer_id column in conversations' as table_check,
...
UNION ALL
SELECT 
  'customer_channel_preferences' as table_check,
  CASE WHEN ist.table_name IS NOT NULL THEN 'EXISTS' ELSE 'MISSING' END as status
FROM information_schema.tables ist
WHERE ist.table_schema = 'public' AND ist.table_name = 'customer_channel_preferences'
UNION ALL
SELECT 
  'email unique constraint' as table_check,
...
WHERE tc.table_schema = 'public' 
  ...
  AND tc.constraint_name = 'customer_profiles_email_key';
