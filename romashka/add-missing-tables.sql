-- Add Missing Security and Analytics Tables to Existing Schema
-- This adds only the missing tables to your current database setup
...
-- Security sessions (for tracking user sessions)
...
-- Security incidents (for logging security events)
...
-- Compliance checks (for defining compliance rules)
...
-- Compliance results (for storing compliance check results)
...
-- Security policies (consistent with your existing pattern)
...
CREATE POLICY "Allow authenticated access" ON compliance_checks
    FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Allow authenticated access" ON compliance_results
    FOR ALL USING (auth.role() = 'authenticated');

-- Analytics policies (allow authenticated users to view analytics)
CREATE POLICY "Allow authenticated access" ON daily_analytics
    FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Allow authenticated access" ON conversation_analytics
    FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Allow authenticated access" ON performance_metrics
    FOR ALL USING (auth.role() = 'authenticated');
...
-- Use your existing update_updated_at_column() function
CREATE TRIGGER trigger_update_compliance_checks_updated_at
...
-- ================================
-- GRANT PERMISSIONS (matching your existing pattern)
-- ================================
...
GRANT ALL ON realtime_metrics TO authenticated;
...
SELECT '✅ Missing tables have been added successfully to your existing schema!' as result;