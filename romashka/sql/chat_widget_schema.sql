...
-- Insert default workflow if not exists
INSERT INTO workflows (id, user_id, name, description, steps, connections)
SELECT 
...
WHERE NOT EXISTS (
  SELECT 1 FROM workflows WHERE id = 'default-workflow'::uuid
);

-- Insert default widget configuration for existing users
