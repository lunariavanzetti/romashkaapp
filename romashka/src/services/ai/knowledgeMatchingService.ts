...
class KnowledgeMatchingService {
  ...
  /**
   * Find answer using custom system and user prompts (for integration-enhanced responses)
   */
  async findAnswerWithCustomPrompts(request: {
    systemPrompt: string;
    userPrompt: string;
  }): Promise<KnowledgeMatchResponse> {
    try {
      console.log('🔍 AI Custom Prompt Analysis');
      ...
      console.log('✅ AI custom prompt response generated:', answer.substring(0, 100) + '...');
      
      // Estimate confidence based on answer quality
      const confidence = this.estimateConfidence(answer, 'custom prompt query');
      ...
      console.error('❌ Custom prompt knowledge matching error:', error);
      return { answer: null, confidence: 0 };
    }
  }
