...
-- Run this after executing fix-customer-id-error.sql to verify everything works
...
FROM pg_policies 
WHERE schemaname = 'public' 
  AND tablename IN ('customer_profiles', 'customer_channel_preferences')
ORDER BY tablename;
...
SELECT 
  ...
  (
    SELECT COUNT(*) 
    FROM information_schema.columns 
    WHERE table_name = ist.table_name AND table_schema = 'public'
  ) as column_count,
